...
DATADIR = '/Users/markant/data/CompetitiveSampling/rttb-v3'
KEYFILE = '%s/rttb-v3-KEY.txt' % DATADIR
...
class Subject:
    ...
    def trainingdata(self):
        ...
        resp_low = [x[3] for x in filter(lambda d: d[2]=='lowest', td)]
        resp_high = [x[3] for x in filter(lambda d: d[2]=='highest', td)]
        ...
        def common(arr):
            ...
            counts = [arr.count(x) for x in values]
            common = [counts[0] > counts[1], counts[0] < counts[1]]

            return {'values': values,
                    'counts': counts,
                    'common': common}

        c = map(common, samples)

        correct = 0
        for i in range(4):

            # if the low outcome was common, check that
            if c[i]['common'][0]:
                #print int(resp_low[i]), c[i]['values'][0]
                correct += float(resp_low[i]) == c[i]['values'][0]
            else:
                #print int(resp_high[i]), c[i]['values'][1]
                correct += float(resp_high[i]) == c[i]['values'][1]
    ...
    def num_gains(self, gid):
        options = self.options(gid)
        return np.sum(np.array([options[opt][-1] for opt in options]) > 0)
