...
def gethspacefile(subj): return hspace_file(HSPACE_NAMES[HSPACE_CB[getcondition(subj)[1]][1]])
...
def gamesplayed(subj=None, block=0):
    return list(where(array([len(loadsamples(subj=subj, block=block, game=g)) for g in range(N_GAMES_PER_BLOCK)]) > 1)[0])
...
if __name__=="__main__":
    subj = 171
    print loadgameboard(subj=subj, game=0)
    print loadsamples(subj=subj, game=0)
    print gamesplayed(subj=subj)
    print gethspacefile(subj)
    print dummy
